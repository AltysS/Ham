...
let currentActiveTabContent = document.querySelector(
  `.ourServiceTabData[data-content=${currentActiveTabTitle.dataset.servicetitle}]`
);
...
const changeTabContent = (prevActiveTab) => {
  ...
  currentActiveTabContent = document.querySelector(
    `.ourServiceTabData[data-content=${currentActiveTabTitle.dataset.servicetitle}]`
  );
...
let currentActiveFilterBtn = document.querySelector(
  ".ourProjectFilterTextActive"
);
...
const filterImgContent = (array) => {
  let currentDataToShow = document.querySelector(".ourProjectFilterTextActive")
    .dataset.filter;
...
const addPortolioImg = (counter) => {
  for (let i = portfolioImgCounter; i < portfolioImgCounter + 3; i++) {
    portfolioImgContainer.insertAdjacentHTML(
      "beforeend",
    ...
    <img src="./IMG/main/portfolio/graphicDesign/graphic-design${
      i + 1
    }.jpg" alt="#"> 
    ...
    <img src="./IMG/main/portfolio/landingPage/landing-page${
      i + 1
    }.jpg" alt="#"> 
...
portfolioLoadMoreBtn.addEventListener("click", (e) => {
  if (
    (e.target.closest("button") && portfolioImgCounter <= 9) ||
    e.target.closest(".сrossDiv")
  ) {
...
const specialistsData = {
  ...
  "JANNET PINK": {
    about:
      "ser experience (UX) design is the process design teams use to create products that provide meaningful and relevant experiences to users. This involves the design of the entire process of acquiring and integrating the product, including aspects of branding, design, usability and function.",
  ...
  "MARRET BRANTEN": {
    about:
      "These tasks can vary greatly from one organization to the next, but they always demand designers to be the users’ advocate and keep the users’ needs at the center of all design and development efforts. That’s also why most UX designers work in some form of user-centered work process, and keep channeling their best-informed efforts until they address all of the relevant issues and user needs optimally.",
    specialization: "UI DESIGNER",
    photo: "./IMG/main/aboutHam/specialists/specialist3.png",
  },
};

let currentSpecialistDiv = document.querySelector(
  ".activeSpecialistInformation"
);
...
const specialistPosition = document.querySelector(
  ".aboutTheHamSpecialistPosition"
);
...
const sliderTabListener = document.querySelector(
  ".aboutTheHamspecialistPhotoSlider"
);
...
const changeActiveSpecialist = (specialistObjInfo, eventTarget) => {
  ...
  setTimeout(() => {
    ...
    let currentSpecialistImg = document.querySelector(
      ".activeSpecialistInformation > img"
    ).dataset.name;
    ...
    bigSpecialistPicture.setAttribute(
      "src",
      `${specialistObjInfo[currentSpecialistImg].photo}`
    );
  }, 700);
};
...
sliderTabListener.addEventListener("click", (e) => {
  if (
    e.target.closest(".aboutTheHamspecialistPhotoCircle") !==
      currentSpecialistDiv &&
    e.target.closest(".aboutTheHamspecialistPhotoCircle")
  ) {
...
document.addEventListener("DOMContentLoaded", () => {
  addPortolioImg(portfolioImgCounter);
  changeActiveSpecialist(
    specialistsData,
    document.querySelector(".activeSpecialistInformation")
  );
});
...
const sliderParentDiv = (nextElement) => {
  Array.from(
    document.querySelectorAll(`.aboutTheHamspecialistPhotoCircle > img`)
  ).find((element) => {
...
sliderTabListener.addEventListener("click", (e) => {
  ...
  if (e.target.closest(".swipeSlider")) {
    ...
    setTimeout(() => {
      ...
      bigSpecialistPicture.setAttribute(
        "src",
        `${specialistsData[currentSpecialistImg].photo}`
      );
    }, 700);
  }
});
